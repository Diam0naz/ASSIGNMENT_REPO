...
function numberDept(arr) {
  for (let i in studentDB) {
    ...
    } else if (studentDB[i] === "ENGINEERING") {
      engineering.push(studentDB[i])
    }
  }
  console.log(agriculture);
  console.log(`There are ${agriculture.length}`);
}
...
function genderSearch(sex) {
  sex = sex.toUpperCase();

  let search = studentDB.find((student) => student.gender === sex);
  //console.log(search);
  if (search) {
    console.log(`The student ${search.name} is a ${search.gender}`);
  } else {
    console.log("Student not found");
  }
}
...
function createNewStudent(name, age, gender, course, stateOfOrigin, gradIng) {
  let studentId = studentDB.length + 1;
...
function updateStudentDataById(id, course, gradIng) {
  //SEARCH THE studentDB TO GET THE ELEMENT WHICH HOLDS THE CORRECT ID
  for (i in studentDB) {
    if (studentDB[i].id === id) {
      let newCourse = course;
      studentDB[i].course = newCourse;
    }
    if (studentDB[i].id === id) {
      let newGrade = gradIng;
      studentDB[i].gradIng = newGrade;
    }
  }
  console.log(studentDB[i]);
  //IF STUDENT RETRIEVAL IS SUCCESSFUL, I WILL NOW CHANGE THE VALUE OF THE TARGET KEY DYNAMICALLY
}

updateStudentDataById(20, "LAW", "THIRDCLASS");
...
createNewStudent("DAVIS", 25, "M", "LAW", "KANO", "FIRSTCLASS");

createNewStudent("JENNY", 26, "F", "PSYCHOLOGY", "JOS", "SECONDCLASSS LOWER");
